...
Conversely, the simplicity of using a value known by its position (like the fictitious battery level stored in x[0]) has its merits. I could cover a lot of use cases by making the ESP32's 32-bit GPIO_IN_REG register available as x[0]..x[3] and ADC1 and ADC2 values as x[4]..x[7]. Maybe even devote some space for the value of `millis()` to provide uptime information. These x[] values can be mapped to meaningful entities using the ESPHome and Home Assistant configurations.
