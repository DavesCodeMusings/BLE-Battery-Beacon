...
Conversely, the simplicity of using a value known by its position (like the fictitious battery level stored in x[0]) has its merits. I could cover a lot of use cases by storing the ESP32's 32-bit GPIO_IN_REG register in x[0]..x[3] and ADC1 and ADC2 values in x[4]..x[7]. Maybe even devote some space for the value of `millis()` to provide uptime information. The meaning of these x[] values can be mapped in the ESPHome and Home Assistant configurations.
