...
void loop() {
  ...
  if (central) {
    ...
    while (central.connected()) {
      ...
      snprintf(batteryPercentString, 4, "%d%%", battery);
      Serial.print("Battery level: ");
      Serial.println(batteryPercentString);
  ...
  delay(100);  // Throttle updates, balancing reponsiveness vs. CPU usage.
  char battPct[5];
  snprintf(battPct, 5, "%3d%%", battery);
  byte data[11] = { 0xA3, 0x09, 0x42, 0x41, 0x54, 0x54, 0x3A, battPct[0], battPct[1], battPct[2], battPct[3]};
